...
public class DiscreteDynamicsWorld extends DynamicsWorld {
		...
		if (getDebugDrawer() != null && (getDebugDrawer().getDebugMode()
			...
			for (RaycastVehicle vehicle : vehicles) {
				for (int v = 0; v < vehicle.getNumWheels(); v++) {
					...
					axle.set(vehicle.getWheelInfo(v).worldTransform.basis.getElement(0, vehicle.getRightAxis()),
							vehicle.getWheelInfo(v).worldTransform.basis.getElement(1, vehicle.getRightAxis()),
							vehicle.getWheelInfo(v).worldTransform.basis.getElement(2, vehicle.getRightAxis()));

					// m_vehicles[i]->getWheelInfo(v).m_raycastInfo.m_wheelAxleWS
					// debug wheels (cylinders)
					tmp.add(wheelPosWS, axle);
					debugDrawer.drawLine(wheelPosWS, tmp, wheelColor);
					debugDrawer.drawLine(wheelPosWS, vehicle.getWheelInfo(v).raycastInfo.contactPointWS, wheelColor);
				}
			}
	...
	public int stepSimulation(float timeStep, int maxSubSteps, float fixedTimeStep) {
		...
		try {
			...
			if (numSimulationSubSteps != 0) {
				...
				// clamp the number of substeps, to prevent simulation grinding spiralling down
				// to a halt
				int clampedSimulationSteps = (numSimulationSubSteps > maxSubSteps) ? maxSubSteps
	...
	private static class InplaceSolverIslandCallback extends SimulationIslandManager.IslandCallback {
		...
		public void processIsland(ObjectArrayList<CollisionObject> bodies, int numBodies,
			...
			if (islandId < 0) {
				...
				// only call solveGroup if there is some work: avoid virtual function call, its
	...
	public void debugDrawObject(Transform worldTransform, CollisionShape shape, Vector3f color) {
		...
		// {
		// const btCompoundShape* compoundShape = static_cast<const
		...
		// btScalar radius = sphereShape->getMargin();//radius doesn't include the
		...
		// {
		// const btCapsuleShape* capsuleShape = static_cast<const
	...
	// private static class DebugDrawcallback implements TriangleCallback,
	...
	//
	// public DebugDrawcallback(IDebugDraw debugDrawer, Transform worldTrans,
	// Vector3f color) {
	// this.debugDrawer = debugDrawer;
	...
	// }
	//
	// public void internalProcessTriangleIndex(Vector3f[] triangle, int partId, int
	// triangleIndex) {
	// processTriangle(triangle,partId,triangleIndex);
	// }
	//
	// private final Vector3f wv0 = new Vector3f(),wv1 = new Vector3f(),wv2 = new
	// Vector3f();
	//
	// public void processTriangle(Vector3f[] triangle, int partId, int
	// triangleIndex) {
	// wv0.set(triangle[0]);
	...
	private static class ClosestNotMeConvexResultCallback extends ClosestConvexResultCallback {
		...
		public ClosestNotMeConvexResultCallback(CollisionObject me, Vector3f fromA, Vector3f toA,
				OverlappingPairCache pairCache, Dispatcher dispatcher) {
			super(fromA, toA);
			this.me = me;
