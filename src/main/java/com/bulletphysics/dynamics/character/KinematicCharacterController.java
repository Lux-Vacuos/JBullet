 ...
 * world. It uses a {@link GhostObject} and convex sweep test to test for
 * upcoming collisions. This is combined with discrete collision detection to
...
public class KinematicCharacterController extends ActionInterface {
	...
	/**
	 * This should probably be called setPositionIncrementPerSimulatorStep. This is
	 * neither a direction nor a velocity, but the amount to increment the position
	 * each simulation iteration, regardless of dt.
	 * <p>
	 *
	...
	/**
	 * Caller provides a velocity with which the character should move for the given
	 * time period. After the time period, velocity is reset to zero. This call will
	 * reset any walk direction set by {@link #setWalkDirection}. Negative time
	...
	private static class KinematicClosestNotMeConvexResultCallback extends CollisionWorld.ClosestConvexResultCallback {
		...
		public float addSingleResult(CollisionWorld.LocalConvexResult convexResult, boolean normalInWorldSpace) {
			...
			if (normalInWorldSpace) {
				...
				hitCollisionObject.getWorldTransform(new Transform()).basis.transform(convexResult.hitNormalLocal,
						hitNormalWorld);
			}
