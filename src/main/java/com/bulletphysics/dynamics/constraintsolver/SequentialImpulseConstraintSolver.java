...
/**
 * SequentialImpulseConstraintSolver uses a Propagation Method and Sequentially
 * applies impulses. The approach is the 3D version of Erin Catto's GDC 2006
 * tutorial. See http://www.gphysics.com
 * <p>
 * 
 ...
 * <p>
 * 
 * Applies impulses for combined restitution and penetration recovery and to
 * simulate friction.
 * 
 * @author jezek2
 */
public class SequentialImpulseConstraintSolver extends ConstraintSolver {
	...
	private final List<SolverBody> tmpSolverBodyPool = new GlueList<>();
	private final List<SolverConstraint> tmpSolverConstraintPool = new GlueList<>();
	private final List<SolverConstraint> tmpSolverFrictionConstraintPool = new GlueList<>();
	...
	private void resolveSplitPenetrationImpulseCacheFriendly(SolverBody body1, SolverBody body2,
			SolverConstraint contactConstraint, ContactSolverInfo solverInfo) {

		if (contactConstraint.penetration < solverInfo.splitImpulsePenetrationThreshold) {
			BulletStats.gNumSplitImpulseRecoveries++;
			float normalImpulse;

			// Optimized version of projected relative velocity, use precomputed cross
			// products with normal
	 ...
	 */
	private float resolveSingleCollisionCombinedCacheFriendly(SolverBody body1, SolverBody body2,
			SolverConstraint contactConstraint, ContactSolverInfo solverInfo) {
	...
	private float resolveSingleFrictionCacheFriendly(SolverBody body1, SolverBody body2,
			SolverConstraint contactConstraint, ContactSolverInfo solverInfo, float appliedNormalImpulse) {
	...
	protected void addFrictionConstraint(Vector3f normalAxis, int solverBodyIdA, int solverBodyIdB, int frictionIndex,
			ManifoldPoint cp, Vector3f rel_pos1, Vector3f rel_pos2, CollisionObject colObj0, CollisionObject colObj1,
			float relaxation) {
		...
		// btScalar denom0 =
		// rb0->computeImpulseDenominator(pos1,solverConstraint.m_contactNormal);
		// btScalar denom1 =
		// rb1->computeImpulseDenominator(pos2,solverConstraint.m_contactNormal);
		// #else
	...
	public float solveGroupCacheFriendlySetup(List<CollisionObject> bodies, int numBodies,
			List<PersistentManifold> manifoldPtr, int manifold_offset, int numManifolds,
			List<TypedConstraint> constraints, int constraints_offset, int numConstraints, ContactSolverInfo infoGlobal,
		...
		try {
			...
			// if (1)
			{
				...
				/*
				 * { for (int i=0;i<numBodies;i++) { btRigidBody* rb =
				 * btRigidBody::upcast(bodies[i]); if (rb && (rb->getIslandTag() >= 0)) {
				 * btAssert(rb->getCompanionId() < 0); int solverBodyId =
				 * m_tmpSolverBodyPool.size(); btSolverBody& solverBody =
				 * m_tmpSolverBodyPool.expand(); initSolverBody(&solverBody,rb);
						...
						for (int j = 0; j < manifold.getNumContacts(); j++) {
							...
							if (cp.getDistance() <= 0f) {
...

								{
									...
									solverConstraint.penetration = Math.min(cp.getDistance() + infoGlobal.linearSlop,
											0f);
									...
									if ((infoGlobal.solverMode & SolverMode.SOLVER_USE_WARMSTARTING) != 0) {
										solverConstraint.appliedImpulse = cp.appliedImpulse
												* infoGlobal.warmstartingFactor;
										if (rb0 != null) {
...

									{
										SolverConstraint frictionConstraint1 = tmpSolverFrictionConstraintPool
												.get(solverConstraint.frictionIndex);
										if ((infoGlobal.solverMode & SolverMode.SOLVER_USE_WARMSTARTING) != 0) {
											frictionConstraint1.appliedImpulse = cp.appliedImpulseLateral1
													* infoGlobal.warmstartingFactor;
											if (rb0 != null) {
									...
									}
									{
										SolverConstraint frictionConstraint2 = tmpSolverFrictionConstraintPool
												.get(solverConstraint.frictionIndex + 1);
										if ((infoGlobal.solverMode & SolverMode.SOLVER_USE_WARMSTARTING) != 0) {
											frictionConstraint2.appliedImpulse = cp.appliedImpulseLateral2
													* infoGlobal.warmstartingFactor;
											if (rb0 != null) {
	...
	public float solveGroupCacheFriendlyIterations(List<CollisionObject> bodies, int numBodies,
			List<PersistentManifold> manifoldPtr, int manifold_offset, int numManifolds,
			List<TypedConstraint> constraints, int constraints_offset, int numConstraints, ContactSolverInfo infoGlobal,
			IDebugDraw debugDrawer/* ,btStackAlloc* stackAlloc */) {
				...
				for (iteration = 0; iteration < infoGlobal.numIterations; iteration++) {
...

					{
						...
						for (j = 0; j < numFrictionPoolConstraints; j++) {
							SolverConstraint solveManifold = tmpSolverFrictionConstraintPool
									.get(orderFrictionConstraintPool.get(j));
					...
					for (iteration = 0; iteration < infoGlobal.numIterations; iteration++) {
						{
							...
							int j;
							for (j = 0; j < numPoolConstraints; j++) {
								SolverConstraint solveManifold = tmpSolverConstraintPool
										.get(orderTmpConstraintPool.get(j));
	...
	public float solveGroupCacheFriendly(List<CollisionObject> bodies, int numBodies,
			List<PersistentManifold> manifoldPtr, int manifold_offset, int numManifolds,
			List<TypedConstraint> constraints, int constraints_offset, int numConstraints, ContactSolverInfo infoGlobal,
			IDebugDraw debugDrawer/* ,btStackAlloc* stackAlloc */) {
		...
		for (int j = 0; j < numPoolConstraints; j++) {
			...
			pt.appliedImpulseLateral1 = tmpSolverFrictionConstraintPool
					.get(solveManifold.frictionIndex + 1).appliedImpulse;
		...
		// printf("m_tmpSolverConstraintPool.size() =
		// %i\n",m_tmpSolverConstraintPool.size());

		/*
		 * printf("m_tmpSolverBodyPool.size() = %i\n",m_tmpSolverBodyPool.size());
		 * printf("m_tmpSolverConstraintPool.size() = %i\n",m_tmpSolverConstraintPool.
		 * size()); printf("m_tmpSolverFrictionConstraintPool.size() = %i\n"
	 ...
	 */
	@Override
	public float solveGroup(List<CollisionObject> bodies, int numBodies, List<PersistentManifold> manifoldPtr,
			int manifold_offset, int numManifolds, List<TypedConstraint> constraints, int constraints_offset,
		...
		try {
			...
			if ((infoGlobal.solverMode & SolverMode.SOLVER_CACHE_FRIENDLY) != 0) {
				...
				float value = solveGroupCacheFriendly(bodies, numBodies, manifoldPtr, manifold_offset, numManifolds,
						constraints, constraints_offset, numConstraints, infoGlobal, debugDrawer/* ,stackAlloc */);
				return value;
			}
					...
					if ((infoGlobal.solverMode & SolverMode.SOLVER_RANDMIZE_ORDER) != 0) {
						if ((iteration & 7) == 0) {
							for (j = 0; j < totalPoints; ++j) {
								// JAVA NOTE: swaps references instead of copying values (but that's fine in
								// this context)
	...
	protected void prepareConstraints(PersistentManifold manifoldPtr, ContactSolverInfo info, IDebugDraw debugDrawer) {
		...
		// only necessary to refresh the manifold once (first iteration). The
		// integration is done outside the loop
		{
			// #ifdef FORCE_REFESH_CONTACT_MANIFOLDS
			...
			for (int i = 0; i < numpoints; i++) {
				...
				if (cp.getDistance() <= 0f) {
					...
					jac.init(mat1, mat2, rel_pos1, rel_pos2, cp.normalWorldOnB,
					...
					if (cpd != null) {
						...
						if (cpd.persistentLifeTime != cp.getLifeTime()) {
							// printf("Invalid: cpd->m_persistentLifeTime = %i cp.getLifeTime() =
							...
							// printf("Persistent: cpd->m_persistentLifeTime = %i cp.getLifeTime() =
							// %i\n",cpd->m_persistentLifeTime,cp.getLifeTime());
						}
					} else {
						...
						cpd.persistentLifeTime = cp.getLifeTime();
						// printf("CREATED: %x . cpd->m_persistentLifeTime = %i cp.getLifeTime() =
					...
					// Dependent on Rigidbody A and B types, fetch the contact/friction response
					// func
	...
	public float solveCombinedContactFriction(RigidBody body0, RigidBody body1, ManifoldPoint cp,
			ContactSolverInfo info, int iter, IDebugDraw debugDrawer) {
		float maxImpulse = 0f;

		{
			if (cp.getDistance() <= 0f) {
				{
					// btConstraintPersistentData* cpd = (btConstraintPersistentData*)
	...
	/**
	 * Advanced: Override the default contact solving function for contacts, for
	 * certain types of rigidbody<br>
	...
	/**
	 * Advanced: Override the default friction solving function for contacts, for
	 * certain types of rigidbody<br>
	 * See RigidBody.contactSolverType and RigidBody.frictionSolverType
	 */
