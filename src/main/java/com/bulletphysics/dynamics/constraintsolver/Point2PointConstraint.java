...
/**
 * Point to point constraint between two rigid bodies each with a pivot point
...
public class Point2PointConstraint extends TypedConstraint {
	...
	public void buildJacobian() {
		...
		for (int i = 0; i < 3; i++) {
			...
			jac[i].init(tmpMat1, tmpMat2, tmp1, tmp2, normal, rbA.getInvInertiaDiagLocal(new Vector3f()),
	...
	public void solveConstraint(float timeStep) {
		...
		// btVector3 angvelA = m_rbA.getCenterOfMassTransform().getBasis().transpose() *
		// m_rbA.getAngularVelocity();
		// btVector3 angvelB = m_rbB.getCenterOfMassTransform().getBasis().transpose() *
		...
		for (int i = 0; i < 3; i++) {
			 ...
			 * m_jac[i].getRelativeVelocity(m_rbA.getLinearVelocity(),angvelA,
