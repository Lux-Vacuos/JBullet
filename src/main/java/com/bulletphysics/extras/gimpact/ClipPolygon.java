...
class ClipPolygon {
	 ...
	 */
	public static void plane_clip_polygon_collect(Vector3f point0, Vector3f point1, float dist0, float dist1,
			List<Vector3f> clipped, int[] clipped_count) {
	 ...
	 */
	public static int plane_clip_polygon(Vector4f plane, List<Vector3f> polygon_points, int polygon_point_count,
			List<Vector3f> clipped) {
		...
		for (int i = 1; i < polygon_point_count; i++) {
			...
			plane_clip_polygon_collect(polygon_points.get(i - 1), polygon_points.get(i), olddist, dist, clipped,
					clipped_count);
		...
		plane_clip_polygon_collect(polygon_points.get(polygon_point_count - 1), polygon_points.get(0), olddist,
	 ...
	 * @return the count of the clipped counts
	 */
	public static int plane_clip_triangle(Vector4f plane, Vector3f point0, Vector3f point1, Vector3f point2,
			List<Vector3f> clipped) {
		ArrayPool<int[]> intArrays = ArrayPool.get(int.class);
