...
public class HullLibrary {
	...
	private List<Tri> tris = new GlueList<>();
	...
	public boolean createConvexHull(HullDesc desc, HullResult result) {
		...
		List<Vector3f> vertexSource = new GlueList<>();
		...
		if (ok) {
			...
			if (ok) {
				...
				// vertex table.
				List<Vector3f> vertexScratch = new GlueList<>();
	...
	// representation.
	private void bringOutYourDead(List<Vector3f> verts, int vcount, List<Vector3f> overts, int[] ocount,
			IntArrayList indices, int indexcount) {
	...
	private boolean cleanupVertices(int svcount, List<Vector3f> svertices, int stride, int[] vcount, // output
																										// number
																										// of
																										// vertices
