...
/**
 * PersistentManifold is a contact point cache, it stays persistent as long as
 * objects are overlapping in the broadphase. Those contact points are created
 * by the collision narrow phase.
 * <p>
 * 
 * The cache can be empty, or hold 1, 2, 3 or 4 points. Some collision
 * algorithms (GJK) might only add one point at a time, updates/refreshes old
 * contact points, and throw them away if necessary (distance becomes too
 * large).
 * <p>
 * 
 * Reduces the cache to 4 points, when more then 4 points are added, using
 * following rules: the contact point with deepest penetration is always kept,
 * and it tries to maximize the area covered by the points.
 * <p>
 * 
...
public class PersistentManifold {
	...
	// private int findContactPoint(ManifoldPoint unUsed, int numUnused,
	// ManifoldPoint pt);
	...
	public void refreshContactPoints(Transform trA, Transform trB) {
		...
		for (i = getNumContacts() - 1; i >= 0; i--) {
			...
			if (!validContactDistance(manifoldPoint)) {
				...
				// contact also becomes invalid when relative movement orthogonal to normal
				// exceeds margin
