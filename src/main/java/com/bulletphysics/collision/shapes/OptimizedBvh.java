...
/**
 * OptimizedBvh store an AABB tree that can be quickly traversed on CPU (and
 * SPU, GPU in future).
 * 
 * @author jezek2
 */
public class OptimizedBvh implements Serializable {
	...
	private final List<OptimizedBvhNode> leafNodes = new GlueList<>();
	private final List<OptimizedBvhNode> contiguousNodes = new GlueList<>();
	...
	protected final List<BvhSubtreeInfo> SubtreeHeaders = new GlueList<>();
	...
	private static class QuantizedNodeTriangleCallback extends InternalTriangleIndexCallback {
		...
		public void internalProcessTriangleIndex(Vector3f[] triangle, int partId, int triangleIndex) {
			...
			triangleNodes.setEscapeIndexOrTriangleIndex(nodeId,
					(partId << (31 - MAX_NUM_PARTS_IN_BITS)) | triangleIndex);
		}
	}

	public void build(StridingMeshInterface triangles, boolean useQuantizedAabbCompression, Vector3f _aabbMin,
			Vector3f _aabbMax) {
		...
		if (useQuantization && SubtreeHeaders.size() == 0) {
			...
			subtree.subtreeSize = quantizedContiguousNodes.isLeafNode(0) ? 1
					: quantizedContiguousNodes.getEscapeIndex(0);
		}
	...
	protected boolean testQuantizedAabbAgainstQuantizedAabb(long aabbMin1, long aabbMax1, long aabbMin2,
			long aabbMax2) {
	...
	protected int sortAndCalcSplittingIndex(int startIndex, int endIndex, int splitAxis) {
		...
		// sort leafNodes so all values larger then splitValue comes first, and smaller
		// values start from 'splitIndex'.
		for (i = startIndex; i < endIndex; i++) {
			// Vector3f center = new Vector3f();
			center.add(getAabbMax(i), getAabbMin(i));
			center.scale(0.5f);
	...
	protected void walkRecursiveQuantizedTreeAgainstQueryAabb(QuantizedBvhNodes currentNodes, int currentNodeId,
	...
	protected void walkStacklessQuantizedTreeAgainstRay(NodeOverlapCallback nodeCallback, Vector3f raySource,
			Vector3f rayTarget, Vector3f aabbMin, Vector3f aabbMax, int startNodeIndex, int endNodeIndex) {
		assert (useQuantization);
		...
		while (curIndex < endNodeIndex) {
			...
			if (boxBoxOverlap) {
				...
				// #if 0
				// bool ra2 = btRayAabb2 (raySource, rayDirection, sign, bounds, param, 0.0,
				// lambda_max);
				// bool ra = btRayAabb (raySource, rayTarget, bounds[0], bounds[1], param,
				// normal);
				// if (ra2 != ra)
				// {
				...
				// #endif
				// #ifdef RAYAABB2
				// rayBoxOverlap = AabbUtil2.rayAabb2 (raySource, rayDirection, sign, bounds,
				// param, 0.0, lambda_max);
				// #else
	...
	protected void walkStacklessQuantizedTree(NodeOverlapCallback nodeCallback, long quantizedQueryAabbMin,
			long quantizedQueryAabbMax, int startNodeIndex, int endNodeIndex) {
		assert (useQuantization);
	...
	public void reportBoxCastOverlappingNodex(NodeOverlapCallback nodeCallback, Vector3f raySource, Vector3f rayTarget,
		...
		if (fast_path) {
			...
			/*
			 * Slow path: Construct the bounding box for the entire box cast and send that
			 * down the tree
			 */
