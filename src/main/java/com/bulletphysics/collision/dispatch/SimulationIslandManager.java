...
public class SimulationIslandManager {
	...
	private final List<PersistentManifold> islandmanifold = new GlueList<>();
	private final List<CollisionObject> islandBodies = new GlueList<>();
	...
	public void updateActivationState(CollisionWorld colWorld, Dispatcher dispatcher) {
		...
		int index = 0;
		for (CollisionObject collisionObject : colWorld.getCollisionObjectArray()) {
	...
	public void buildAndProcessIslands(Dispatcher dispatcher, List<CollisionObject> collisionObjects,
		...
		try {
			...
			//
			// callback->ProcessIsland(&collisionObjects[0],collisionObjects.size(),manifold,maxNumManifolds,
			// -1);
			// #else
			...
			for (startIslandIndex = 0; startIslandIndex < numElem; startIslandIndex = endIslandIndex) {
				...
				for (endIslandIndex = startIslandIndex; (endIslandIndex < numElem)
						&& (getUnionFind().getElement(endIslandIndex).id == islandId); endIslandIndex++) {
					CollisionObject colObj0 = collisionObjects.get(getUnionFind().getElement(endIslandIndex).sz);
				...
				if (startManifoldIndex < numManifolds) {
					...
					if (curIslandId == islandId) {
						...
						// startManifold = islandmanifold.subList(startManifoldIndex,
						// islandmanifold.size());
						startManifold_idx = startManifoldIndex;

						for (endManifoldIndex = startManifoldIndex + 1; (endManifoldIndex < numManifolds)
								&& (islandId == getIslandId(
										islandmanifold.get(endManifoldIndex))); endManifoldIndex++) {
	...
	public static abstract class IslandCallback {
		public abstract void processIsland(List<CollisionObject> bodies, int numBodies,
				List<PersistentManifold> manifolds, int manifolds_offset, int numManifolds, int islandId);
	}
