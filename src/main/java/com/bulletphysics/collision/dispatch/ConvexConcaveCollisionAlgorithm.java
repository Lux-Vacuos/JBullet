...
/**
 * ConvexConcaveCollisionAlgorithm supports collision between convex shapes and
 * (concave) trianges meshes.
 * 
 * @author jezek2
 */
public class ConvexConcaveCollisionAlgorithm extends CollisionAlgorithm {
	...
	public void processCollision(CollisionObject body0, CollisionObject body1, DispatcherInfo dispatchInfo,
		...
		if (triBody.getCollisionShape().isConcave()) {
			...
			if (convexBody.getCollisionShape().isConvex()) {
				...
				// Disable persistency. previously, some older algorithm calculated all contacts
				// in one go, so you can clear it here.
				// m_dispatcher->clearManifold(m_btConvexTriangleCallback.m_manifoldPtr);

				btConvexTriangleCallback.manifoldPtr.setBodies(convexBody, triBody);

				concaveShape.processAllTriangles(btConvexTriangleCallback,
	...
	public float calculateTimeOfImpact(CollisionObject body0, CollisionObject body1, DispatcherInfo dispatchInfo,
		...
		// quick approximation using raycast, todo: hook up to the continuous collision
		// detection (one of the btConvexCast)
		...
		tmp.sub(convexbody.getInterpolationWorldTransform(new Transform()).origin,
				convexbody.getWorldTransform(new Transform()).origin);
	...
	public static class CreateFunc extends CollisionAlgorithmCreateFunc {
		private final ObjectPool<ConvexConcaveCollisionAlgorithm> pool = ObjectPool
				.get(ConvexConcaveCollisionAlgorithm.class);
