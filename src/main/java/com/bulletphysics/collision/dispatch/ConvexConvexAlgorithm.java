...
public class ConvexConvexAlgorithm extends CollisionAlgorithm {
	...
	public void init(PersistentManifold mf, CollisionAlgorithmConstructionInfo ci, CollisionObject body0,
			CollisionObject body1, SimplexSolverInterface simplexSolver, ConvexPenetrationDepthSolver pdSolver) {
		super.init(ci);
	...
	public void processCollision(CollisionObject body0, CollisionObject body1, DispatcherInfo dispatchInfo,
		...
		// #ifdef USE_BT_GJKEPA
		// btConvexShape*
		// shape0(static_cast<btConvexShape*>(body0->getCollisionShape()));
		// btConvexShape*
		// shape1(static_cast<btConvexShape*>(body1->getCollisionShape()));
	...
	@Override
	public float calculateTimeOfImpact(CollisionObject col0, CollisionObject col1, DispatcherInfo dispatchInfo,
			ManifoldResult resultOut) {
		...
		// For proper CCD, better accuracy and handling of 'allowed' penetration should
		// be added
		...
		// Convex0 against sphere for Convex1
		{
			...
			SphereShape sphere1 = new SphereShape(col1.getCcdSweptSphereRadius()); // todo: allow non-zero sphere sizes,
		...
		// Sphere (for convex0) against Convex1
		{
			...
			SphereShape sphere0 = new SphereShape(col0.getCcdSweptSphereRadius()); // todo: allow non-zero sphere sizes,
																					// for better approximation
			ConvexCast.CastResult result = new ConvexCast.CastResult();
			VoronoiSimplexSolver voronoiSimplex = new VoronoiSimplexSolver();
			// SubsimplexConvexCast ccd0(&sphere,min0,&voronoiSimplex);
